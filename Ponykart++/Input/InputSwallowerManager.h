...
public:
	...
	/** If it's part of a UI, this should be the class that manages that UI thingy.
	...
	// @param swallower The object that "manages" the thing doing the swallowing. For example if the swallower is part of the UI, this should be that UI thingy's "manager". */
	...
	/// Is the current input swallowed or not?
	/** This method is the same as IsSwallowed(object) but without the extra condition.
